...
export const verifyOtp = createAsyncThunk(
  'verifyOtp',
  async ({phoneNumber, otp, type, navigation,setEnableSubmitOtp}, {dispatch}) => {
    try {
      dispatch(startLoader(true));
      const res = await verifyOtpService({phoneNumber, otp, type, navigation,dispatch});
...
export const verifyLoginOtp = createAsyncThunk(
  ...
  async ({companyId, otp, navigation,setEnableSubmitOtp}, {dispatch}) => {
    try {
      dispatch(startLoader(true));
      const res = await verifyLoginOtpService({companyId, otp, navigation,dispatch});
      console.log('response', res.data);
      await AsyncStorage.setItem('token', res.data.data.accessToken);
      await AsyncStorage.setItem('refreshToken', res.data.data.refreshToken);
      setEnableSubmitOtp(false)
...
export const kycUpdate = createAsyncThunk(
  'kycUpdate',
  async (
    {aadharNo, panNo, gstinNo, fssaiNo,navigation},
    {dispatch},
  ) => {
    try {
      dispatch(startLoader(true));
